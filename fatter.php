<? 
/*
FATTER Version 2.2.3:
- removed all updates for domain in post related content based on logic
*this feature was requested but in the end too many problems have been caused so it needs to be removed. The domain update will, once again, only update the 2 fields in options until a better solution is made

Need to do:
- Enable Auto Destruct. Proven they can't be trusted
- Update 'Archive the Things' to dump to root and include in tarball
...
//ini_set('display_errors','1'); - used for debugging the debugging tool
...
******************************************************/
if(isset($_GET['logs'])){
		echo '<HTML> <header> <title> FATT LOG REVIEW! </title> <H1>WordPress Debug Log:</H1><H4>This field is showing you the WordPress debug.log file held within wp-content.<p> <iframe src="./wp-content/debug.log"  width="100%" height="30%"></iframe><p> <p> <H1>File Scan:</H1><p> <iframe src="./wp-content/file_scan.log" width="100%" height="30%"></iframe><p> <H1>Database Scan:</H1><p> <iframe src="./wp-content/db_scan.log" width="100%" height="30%"></iframe>';
...
//if "email" is not filled out, display the form
  {
	$apsub = 'apsubmit@secureserver.net'; 
	$subject = time().' - Managed WordPress Mail Check - delete me -';//Added the time() so we can track the message easier
	$message = 'This message was generated by an automated tool from Godaddy support. If you can read this, feel free to delete this message';
if (mail($to,$subject,$message))
	{
	echo "Email sent! PHP mail() function is working properly!";
	}
...
#phpinfo body,body,html,div{cursor:default;background-color:#13488A}#phpinfo body,#phpinfo pre,h1,h2,td,th{font-family:'PT Mono',Tahoma,Geneva,sans-serif}#phpinfo .e,#phpinfo .h{font-weight:700;color:#FFF}#phpinfo body,body{color:#FFF}#phpinfo pre{margin:2px}#phpinfo a:link{color:#FFF;text-decoration:none;background-color:#fff}#phpinfo a:hover{text-decoration:underline}#phpinfo table{border-collapse:collapse}#phpinfo .center{text-align:center}#phpinfo .center table{margin-left:auto;margin-right:auto;text-align:left}#phpinfo .center th{text-align:center!important}#phpinfo td,th{border:1px solid #222;font-size:90%;vertical-align:middle}#phpinfo h1{font-size:150%}#phpinfo h2{font-size:125%}#phpinfo .p{text-align:center}#phpinfo .e{background-color:#333}#phpinfo .e:hover{background-color:#373737}#phpinfo .h{background-color:#236AC3}#phpinfo .v{background-color:#333;color:#89E14A}#phpinfo .v:hover{background-color:#373737;color:#89E14A}#phpinfo .vr,#phpinfo hr{background-color:#333;color:#FFF}#phpinfo .vr{text-align:right}#phpinfo img{float:right;border:0}#phpinfo hr{width:600px;border:0;height:1px}
</style>
...
ob_end_clean () ;
//$pinfo = preg_replace( '%(class="v")(>)(.*)(<\/td>)%ms', '$1 onclick="tableText(\'$3\')">$3$4', $pinfo );
...
class get_started{
	...
	private function initialize(){
		...
		if(file_exists('wp-config.php')){
			...
			$config['DB_HOST_DEBUG'] = shell_exec("cat wp-config.php | grep DB_HOST | cut -d \' -f 4");
			$config['DB_PORT'] = '';
			...
			$wnode = gethostname();
				...
				$wnode = php_uname('n'); //there may be issues with this on shared environments though, so need to take that into consideration.
				}
				
				if (version_compare(PHP_VERSION, '5.0.0', '<')) {
				$wnode = getenv('HOSTNAME');
				if(!$wnode ) $wnode = trim(`wnode`);
				if(!$wnode ) $wnode = exec('echo $HOSTNAME');//not sure about this
				if(!$wnode ) $wnode = preg_replace('#^\w+\s+(\w+).*$#', '$1', exec('uname -a')); 
				}
			...
			$fin = str_replace('html', $file, $swap);
			...
			foreach($config as $k => $v){
				...
				if(empty($v) && ($k !== 'DB_PORT') && ($k !== 'WP_VER')&& ($k !== 'E_RELAY')){
...
******************************************************/
	class selector {
		...
		public function __construct($selection = ''){
			...
			if(!empty($selection)){
				...
				// Route actions that require the script to die
					...
					$response .= $this->display->set_warning_box('<p>FATT has completed the operation.</p>');
					
					echo $response;
					
					//$this->killFatt();  Auto-destruct removed to make the tool easier to use. If script is left behind this will be re-enabled.
				}
			}
		}		
		/** MySQL Backups **/
		...
		public function select_sql_backup(){
			...
			if(!empty($file_list)){
				...
				$response .= '
					...
					<div class="col-sm-12">
						<button type="submit" class="btn btn-default pull-right"><font color="white">RESTORE BACKUP</font></button>
					</div>
				</form>';
			}else{
		...
		public function select_archive(){
			...
			if(!empty($archive_list)){
				...
				$response .= '
					...
					<div class="col-sm-12">
						<button type="submit" class="btn btn-default pull-right"><font color ="white">UNPACK ARCHIVE</font></button>
					</div>
				</form>';
			}else{
		...
		private function killFatt(){
			...
			$killarch = "find * -type f -name 'FATT-Archive*' -mtime +3 -exec rm {} \;";
			$debug = "wp-config.php.fatt-debug-bak";
			$review = "fatt-review.html";
			$killdebug = "mv wp-config.php wp-config.php.debug; mv ".$debug." wp-config.php; chmod 600 wp-config.php.debug";
			$killreview = "rm fatt-review.html";
		...
		// Cleans up any / all files related to FATTER, including the archives and the MySQL dumps
		...
		public function database_scan(){
			...
			shell_exec($cmd);
				if (file_exists($file)) {
					...
					echo $this->display->set_error_box('<p>Something went wrong while trying to create the output file!</p>');
				}
		}
		...
		public function file_scan(){
			...
			$file = basename(__FILE__);
				...
				if (file_exists($fscan)) {
					...
					$response = $this->display->set_error_box('<p>Something went wrong while trying to create the output file!</p>');
				}
				echo $response;
		}
		...
		public function sel_f_scan(){
			...
			// dev script for function to enable wp debugging for fatter.php
		...
		$dbconf = "<?php\n/**\n * The base configurations of the WordPress.\n *\n * This file has the following configurations: MySQL settings, Table Prefix,\n * Secret Keys, and ABSPATH. You can find more information by visiting\n * {@link http://codex.wordpress.org/Editing_wp-config.php Editing wp-config.php}\n * Codex page. You can get the MySQL settings from your web host.\n *\n * This file is used by the wp-config.php creation script during the\n * installation. You don't have to use the web site, you can just copy this file\n * to 'wp-config.php' and fill in the values.\n *\n * @package WordPress\n */\n\n// ** MySQL settings - You can get this info from your web host ** //\n/** The name of the database for WordPress */\ndefine('DB_NAME', '".DB_NAME."');\n\n/** MySQL database username */\ndefine('DB_USER', '".DB_USER."');\n\n/** MySQL database password */\ndefine('DB_PASSWORD', '".DB_PASSWORD."');\n\n/** MySQL hostname */\ndefine('DB_HOST', '".DB_HOST_DEBUG."');\n\n/** Database Charset to use in creating database tables. */\ndefine('DB_CHARSET', 'utf8');\n\n/** The Database Collate type. Don't change this if in doubt. */\ndefine('DB_COLLATE', '');\n\n/**#@+\n * Authentication Unique Keys and Salts.\n *\n * Change these to different unique phrases!\n * You can generate these using the {@link https://api.wordpress.org/secret-key/1.1/salt/ WordPress.org secret-key service}\n * You can change these at any point in time to invalidate all existing cookies. This will force all users to have to log in again.\n *\n * @since 2.6.0\n */\ndefine('AUTH_KEY',         '".$authk."');\ndefine('SECURE_AUTH_KEY',  '".$sauthk."');\ndefine('LOGGED_IN_KEY',    '".$logink."');\ndefine('NONCE_KEY',        '".$noncek."');\ndefine('AUTH_SALT',        '".$asalt."');\ndefine('SECURE_AUTH_SALT', '".$sasalt."');\ndefine('LOGGED_IN_SALT',   '".$logsalt."');\ndefine('NONCE_SALT',       '".$nonsalt."');\n\n/**#@-*/\n\n/**\n * WordPress Database Table prefix.\n *\n * You can have multiple installations in one database if you give each a unique\n * prefix. Only numbers, letters, and underscores please!\n */\n\$table_prefix  = '".PREFIX."';\n\n/**\n * For developers: WordPress debugging mode.\n *\n * Change this to true to enable the display of notices during development.\n * It is strongly recommended that plugin and theme developers use WP_DEBUG\n * in their development environments.\n */\n// Enable WP_DEBUG mode\ndefine('WP_DEBUG', true); \ndefine('WP_DEBUG_LOG', true); // Enable Debug logging to the /wp-content/debug.log file\ndefine('WP_DEBUG_DISPLAY', false);@ini_set('display_errors',0);\ndefine('SCRIPT_DEBUG', true);\n\n//define( 'WP_CACHE', true );\nrequire_once( dirname( __FILE__ ) . '/gd-config.php' );\ndefine( 'FS_METHOD', 'direct');\ndefine('FS_CHMOD_DIR', (0705 & ~ umask()));\ndefine('FS_CHMOD_FILE', (0604 & ~ umask()));\n\n\n/* That's all, stop editing! Happy blogging. */\n\n/** Absolute path to the WordPress directory. */\nif ( !defined('ABSPATH') )\n	define('ABSPATH', dirname(__FILE__) . '/');\n\n/** Sets up WordPress vars and included files. */\nrequire_once(ABSPATH . 'wp-settings.php');";
			...
			$move = 'mv ./wp-config.php ./wp-config.php.fatt-debug-bak';
			shell_exec($move);
			...
			if (file_exists("wp-config.php.fatt-debug-bak")) {
					echo $this->display->set_success_box('<p>WordPress Debugging Enabled - Please review ./wp-content/debug.log</p>');
				}else{
					echo $this->display->set_error_box('<p>Something went wrong while trying to backup the configuration file!</p>');
				}
		}
		...
		public function dis_plugins(){
			...
			$curPlug = shell_exec($curPlugins);
				...
				if(trim($curPlug) != "a:0:{}"){
					...
					$unPlug   = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "fatt-active_plugins-toggled" WHERE option_name = "active_plugins"\'';
					shell_exec($unPlug);
					//Create our active plugins
					$fattPlug = $sqLogin.'INSERT INTO '.PREFIX.'options(option_id, option_name, option_value, autoload) VALUES (LAST_INSERT_ID( ), "active_plugins", "a:0:{}", "yes");\''; 
					shell_exec($fattPlug);
				}else{
					//We Should Probably Check to make sure our fattabase entry exists first
					$checkFattPlug = $sqLogin.'SELECT option_name FROM '.PREFIX.'options WHERE option_name = "fatt-active_plugins-toggled"\'';
					$checkFP = shell_exec($checkFattPlug);
					if(!empty($checkFP) && $checkFP != NULL){
						
						//Move our tracks
						$mvFattPlug = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "fatt-active_plugins-remove" WHERE option_name = "active_plugins"\'';
						shell_exec($mvFattPlug);
						
						//Restore their plugins
						$rePlug = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "active_plugins" WHERE option_name = "fatt-active_plugins-toggled"\'';
						shell_exec($rePlug);
						
						//Remove our tracks
						$unFattPlug = $sqLogin.'DELETE FROM '.PREFIX.'options WHERE option_name = "fatt-active_plugins-remove"\'';
						shell_exec($unFattPlug);
					}
				}
			}
		...
		public function default_theme(){
			...
			/////////////////////////////////////////
			$checkFattTemp  = $sqLogin.'SELECT option_name FROM '.PREFIX.'options WHERE option_name = "fatt-template-toggled"\'';
			$checkFT = shell_exec($checkFattTemp);
				if($checkFT != NULL ){
				$response = '<span class="glyphicon glyphicon-ok-circle glyphicon-size-massive" aria-hidden="true"></span>
		...
		public function dis_theme(){
			...
			if(trim($curTemp) != $defaultTheme && trim($curStyle) != $defaultTheme){//open1
				$fattTemp  = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "fatt-template-toggled" WHERE option_name LIKE "template" \'';
				$fattStyle = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "fatt-stylesheet-toggled" WHERE option_name LIKE "stylesheet" \'';
				$fattDefTemp  = $sqLogin.'INSERT INTO '.PREFIX.'options(option_id, option_name, option_value, autoload) VALUES (LAST_INSERT_ID( ), "template", "twentysixteen", "yes");\'';	
				$fattDefStyle = $sqLogin.'INSERT INTO '.PREFIX.'options(option_id, option_name, option_value, autoload) VALUES (LAST_INSERT_ID( ), "stylesheet", "twentysixteen", "yes");\'';
				shell_exec($fattTemp);
				shell_exec($fattStyle);
				shell_exec($fattDefTemp);
				shell_exec($fattDefStyle);
				
				$dtheme = "twentysixteen-fatt.zip";
				$move ="mv ./wp-content/themes/twentysixteen ./wp-content/themes/twentysixteen.bak;";
				$wget = "wget http://bkullman.com/twentysixteen-fatt.zip; unzip twentysixteen-fatt.zip -d ./wp-content/themes/";
				$theme = "./wp-content/themes/twentysixteen";
		
				if (file_exists($dtheme)) {
					//no duplicates - don't trust anyone. Download new.
					$bye = "rm twentysixteen-fatt.zip";
					shell_exec($bye);
				}
			...
			//Now we undo everything we've done
				//We Should Probably Check to make sure our fattabase entry exists first
					$checkFattTemp  = $sqLogin.'SELECT option_id FROM '.PREFIX.'options WHERE option_name = "fatt-template-toggled"\'';
					$checkFattStyle = $sqLogin.'SELECT option_id FROM '.PREFIX.'options WHERE option_name = "fatt-stylesheet-toggled"\'';
					$checkFT = shell_exec($checkFattTemp);
					$checkFS = shell_exec($checkFattStyle);
					if(checkFS != NULL && $checkFT != NULL){
						//Move our tracks
						$mvTemp  = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "fatt-template-remove" WHERE option_name LIKE "template" \'';
						$mvStyle = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "fatt-stylesheet-remove" WHERE option_name LIKE "stylesheet" \'';
						shell_exec($mvTemp);
						shell_exec($mvStyle);						
						
						//Restore their temp and style
						$reTemp  = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "template" WHERE option_name = "fatt-template-toggled"\'';
						$reStyle = $sqLogin.'UPDATE '.PREFIX.'options SET option_name = "stylesheet" WHERE option_name = "fatt-stylesheet-toggled"\'';
						shell_exec($reTemp);
						shell_exec($reStyle);
						
						//Remove our tracks
						$unFattTemp  = $sqLogin.'DELETE FROM '.PREFIX.'options WHERE option_name = "fatt-template-remove" \'';
						$unFattStyle = $sqLogin.'DELETE FROM '.PREFIX.'options WHERE option_name = "fatt-stylesheet-remove" \'';
						shell_exec($unFattTemp);
						shell_exec($unFattStyle);
				}
			}
		}
	}
	...
	class fattArchive{
		...
		*/
		public function create_archive(){
			...
			$time = date("_His_mdY");
			$filename = 'FATT-Archive_'.$dom.''.$time.'.tar.gz';
	...
	class mysqlStuffs{
		...
		*/
		public function killTransient($prefix = PREFIX){
			...
			//executes the command now that it is formed - this will clear the cache
			shell_exec($xbdom);
...
<html lang="en">
	<head>
		...
		<title>FATTER - Fix all the Things in Europe!</title>
		...
		.navbar-fatt{background-color:#ff8100;border-radius:0;border:none;}
		.navbar-brand{color:#FFF;}
		.navbar-brand:hover, .navbar-brand:focus{outline:0;color:#FFF;background-color:#FDA601;}
		#main-content{font-family:"Helvetica Neue","Segoe UI",Segoe,Helvetica,Arial,"Lucida Grande",sans-serif;}
		.btn-default, .btn-default:hover, .btn-default:focus, .btn-default:active, .btn-default.active, .open .dropdown-toggle.btn-default{margin:5px 3px; background-color:#FD7301; border-radius:0; color:#333;font-weight:bold;border-bottom:4px solid #333;}
		...
		.btn-default:hover, .btn-default:focus, .btn-default:active, .btn-default.active, .open .dropdown-toggle.btn-default{background-color:#D62A2A;}
		.nav > li > a:hover, .nav > li > a:focus, .nav-pills > li.active > a, .nav-pills > li.active > a:hover, .nav-pills > li.active > a:focus{background-color:#FDA601; color:#FFF;outline:none;}
		...
		.btn-info:hover, .btn-info:focus, .btn-info:active, .btn-info.active, .open .dropdown-toggle.btn-info{background-color:#FD7301; border-color:#000;}
		.btn-inf {border-color: #000; background-color:#FD7301; border-radius: 0; font-size: 18px; line-height: 20px; margin: 0; padding: 15px 0;  width: 100%;}
		.btn-inf:hover, .btn-info:focus, .btn-info:active, .btn-info.active, .open .dropdown-toggle.btn-info{background-color:#D62A2A; border-color:#000;}
		...
		.box-select:hover{}
		.box-select.active, button-select.active:hover{background-color:#FDA601; color:#FFF; border-color:#e67c00;}
		.alert-heading{background-color:#FF4D00; color:#FFF; border-radius:0;}
		.well{background-color:#FFF; border-radius:0; box-shadow:4px 4px 0 0 rgba(0, 0, 0, 0.1); padding:10px;}
		a.jumbotron-links{color:#000;}
		a.jumbotron-links:hover, a.jumbotron-links:hover .alert{background-color:#FDA601; text-decoration:none;}
		...
		.glyphicon-size-large{font-size:3em;}
					...
					<div class="row">
						<div class="col-md-4 text-center col-no-pad">
							<form role="form" method="post">
								...
								<div id="cache1"><button type="submit" id="cache" class="btn btn-inf"><font color="white">Clear WP Cache (SQL/Varnish)</button></div></font>
							</form>
						</div>
						<div class="col-md-4 text-center col-no-pad">
							<form role="form" method="post">
						...
						<div class="col-md-4 text-center col-no-pad">
							<form role="form" method="post">
								...
								<div id="remove1"><button type="submit" id="remove" class="btn btn-inf"><font color="white">Remove FATTER</button></div></font>
							</form>
						</div>
					</div>
				</div>
                    ...
                    <div class="col-xs-12">
                        <div class="row ux-pagetitle">
                            <div class="col-md-offset-1 col-md-5">
								...
								<h4><b>Absolute Path:  </b><font color ="#1d6ccd"><? echo getcwd(); ?></h4></font>
								<h4><b>Web Node:</b><font color ="#1d6ccd"> <?=W_NODE;?>&nbsp; &nbsp;<?=shell_exec('curl -Is '.SITE_URL.' | grep "X-Cache:"');?></font></h4>
								<h4><b>Email Relays:  </b><font color ="#1d6ccd"><?=E_RELAY;?></h4></font>
								<h4><b>Site Preview:  </b><font color ="#1d6ccd"><a onclick="$('#preview1').toggle(); $('#MainFatt').toggle(); document.getElementById('preFrame').contentWindow.location.reload();var iframe =  document.getElementById('preFrame'); iframe.setAttribute('src', '/?nocache='+d.getTime()); document.getElementById('preFrame').src = document.getElementById('preFrame').src;"><?=$_SERVER['HTTP_HOST'];?></h4></font></a>
								 
                            </div>
                            <div class="col-md-5 pull-right">
                               <h3>WordPress FATTER</h3>
								<h4><b>FATTER Version:</b><font color ="#1d6ccd"> <?=VERSION;?></font></h4>
								<h4><b>FATTER Archive:</b><font color ="#1d6ccd"> <?=$_SESSION['tarball'];?></font></h4>
								<h4><b>FATTER DB Backup:</b><font color ="#1d6ccd"> <?=$_SESSION['sql_backup'];?></font></h4>
							    <a href="./<? echo basename(__FILE__) ?>?logs=<? echo time(); ?>" target="_blank"><font color="FF0000"><b>Click to review logs!</a></b></font><br />
							    <a href="./<? echo basename(__FILE__) ?>?phpinfo=<? echo time(); ?>" target="_blank"><font color="FF0000"><b>Click for PHP Info!</a></b></font><br />
							    <a href="./<? echo basename(__FILE__) ?>?mail=<? echo time(); ?>" target="_blank"><font color="FF0000"><b>Click to test the PHP Mail!</a></b></font><br />
                                    ...
                                    <div class="item active">
                                        <div class="col-xs-12">
                                            ...
                                            <div class="row">
                                                        ...
                                                        <div class="well">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-lg-12">
                                                                    <div class="alert alert-heading small">
                                                                        <p class="list-group-item-text hidden-xs">
                                                                            Create a fatter PHP ini file with a few clicks
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                            </div>
												...
												<a class="jumbotron-links" data-target="#fatt-menu" data-slide-to="5">
                                                    <div class="col-sm-4">
                                                        <div class="well">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-lg-12">
                                                                    <div class="alert alert-heading small">
                                                                        <p class="list-group-item-text hidden-xs">
                                                                           Logs, more useful than you might think
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                            </div>
												...
												<a class="jumbotron-links" data-target="#fatt-menu" data-slide-to="6">
                                                    <div class="col-sm-4">
                                                        <div class="well">
                                                            <h4 class="alert alert-heading">SCAN THE THINGS!</h4>
                                                            <div class="row">
                                                                <div class="col-lg-12 text-center jumbotron">
                                                            ...
                                                            <div class="row">
                                                                <div class="col-lg-12">
                                                                    <div class="alert alert-heading small">
                                                                        <p class="list-group-item-text hidden-xs">
                                                                           Quick Scan for suspicious files to be reviewed
                                                                        </p>
                                                                    </div>
                                                                </div>
                                                            </div>
                                            ...
                                            <div class="row">
                                                <div class="col-md-5">
                                                	<div class="well">
                                                        ...
                                                        <ol>
                                                            <li>Creates a .sql.gz dump within wp-content/</li><p>
												...
												<div class="row">
													<div class="col-sm-6">
														<div class="well">
															...
															<p>
																This option will automagically update the table prefixes AND all of the database entries for capabilities, user_roles, etc. that are necessary for WordPress to operate normally.
															</p>
															<div class="row">
													...
													<div class="col-sm-6">
														<div class="well">
															...
															<p>This option  will update the siteurl and home database values to reflect the domain being currently used in the address bar. Ensure the 'New:' value is correct before updating!</p>
															<div class="row">
																 <div class="col-xs-offset-2 col-xs-8 box-select">
																	<?=$s->get_url();?>
																</div>
														...
														<div class="col-md-4 hidden-xs">
															...
															<table class="table table-hover">
																...
																<tbody>
																	...
																	<tr>
																		<td>4GH Shared Hosting</td>
																		<td>php5.ini when using PHP 5.3</td>
																	</tr>
																</tbody>
															</table>
															...
															<p class="text-info small">
																See vault article 8913 for more information.
															</p>
														</div>
												...
												<div class="row">
													<div class="col-sm-6">
														<div class="well">
															<h3>Debug Things!</h3>
															<p>
																This tool will assist in enabling debugging for troubleshooting purposes. This will enable WordPress/PHP debugging. You can review at:<b> /wp-content/debug.log</b> * please note that this is the default location 
															</p>
															<div class="row">
																<div class="col-xs-offset-2 col-xs-8 box-select">
																	<?=$s->sel_wp_debug();?>
														...
														<div class="well">
															<h3>Toggle Things!</h3>
															<p>
																If you are troubleshooting what is going wrong in a plugin or theme, you have found the right place. Many people may think they need to review their server logs, but you should always start by debugging your application!<p>
																<center><h4>Toggle Plugins and/or Theme on and off</h4></center>
															</p>
															<div class="row">
                                    ...
                                    <div class="item">
                                         <div class="col-sm-12">
                                            <h2 class="alert alert-heading">
											 ...
											 <form name="scan_things" class="form-horizontal" role="form" method="post">
												<div class="row">
													<div class="col-sm-6">
														<div class="well">
															<h3>Scan Files</h3>
															<p>
																This tool will scan for common compromise signatures. It is looking for '<i>Base64, 46esab, eval, gzinflate</i>'. These methods may be being used legitimately. These are <b>intended to be reviewed, not removed. </b>
															</p>
															<div class="row">
																<div class="col-xs-offset-2 col-xs-8 box-select">
																	<?=$s->sel_f_scan();?>
													...
													<div class="col-sm-6">
														<div class="well">
															<h3>Scan Database</h3>
															<p>
															This tool will scan the database to search for common injection methods (iframe, noscript, etc) within the posts table. This can be used to search for 'Pharma' hacks and other suspicious iFrame or display injections.
															</p>
															<div class="row">
																 <div class="col-xs-offset-2 col-xs-8 box-select">
																	<?=$s->sel_db_scan();?>
												...
												<div class="row">
													<div class="col-sm-12 text-center">
														<button type="submit" class="btn btn-default">
														<font color ="white">RUN SCAN</font>
														</button>
													</div>
												</div>
											</form>
		...
		<script>
			$(function(){
				...
				$("form").submit(function(e){
					...
					output.html('<div class="alert alert-info" role="alert" style="margin-bottom:-15px;" >Please wait patiently while I do the needful.</div>');
					$.ajax({
				...
				$('select[name="archive_file"]').on('change', function(){
					...
					if($(this).val().indexOf('FATT-Archive_') !== -1){
